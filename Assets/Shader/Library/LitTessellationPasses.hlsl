...
#define MY_DOMAIN_PROGRAM_INTERPOLATE(fieldName) input[0].fieldName * barycentricCoordinates.x + \
...
#if _VERTEX_COLOR_DISPLACEMENT
	baseHeight = clamp(varyings.VERTEX_COLOR_ID.x - _VertexColorDispBias, 0.0, 1.0);
#endif
#endif
	...
	data.POSITION_ID.xyz += normalize(outputNormal) * SampleHeightWorldCoordinates(positionWS, normalWS) * baseHeight;
#else
	float2 samplingUV = TRANSFORM_TEX(GEOMETRY_UV_CHANNEL, TESSELLATION_TRANSFORM_SOURCE);
	data.POSITION_ID.xyz += normalize(NORMAL_ID) * SampleHeight(samplingUV) * baseHeight;
#endif
#endif
